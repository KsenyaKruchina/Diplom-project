...
const AppLayout = ({ theme, onThemeChange }) => {
  ...
  const renderPage = () => {
    switch (activePage) {
      ...
      case "settings":   return <SystemSettings theme={theme} onThemeChange={onThemeChange} />;
...
// две колонки — Sidebar слева и main справа, которые работаю сообща
  return (
    <div className="app-screen" data-theme={theme}>
      <div className="app-mobile-brand">TEMPERATURA.KZ</div>
...
const AppRouter = () => {
  ...
  const userThemeKey = user ? `app-theme:${user.id ?? user.username}` : null;
  ...
  useEffect(() => {
    ...
    setTheme(savedUserTheme || "dark");
  }, [userThemeKey]);

  useEffect(() => {
    document.body.dataset.theme = theme;
    localStorage.setItem("app-theme", theme);
    if (userThemeKey) localStorage.setItem(userThemeKey, theme);
  }, [theme, userThemeKey]);
